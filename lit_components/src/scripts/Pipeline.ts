...
import { copyMat, imageDataToMat, imageDataToTensor, matToImageData, resizeImage, showImageDataInCanvas, type ImageSize } from "./AiHelpers";
...
export async function runModelPipelineFromFile(model: ClassificationModel, inputSize: number, file: File, canvas: HTMLCanvasElement | undefined) {
	const offscreenContext = await drawFileToOffscreenCanvas(file);
	...
	await loadHaarCascade();
	const faceData = detectFace(originalMat);
	...
	if (canvas) {
		console.log("FaceData", faceMat.size());
		const copy = originalMat.clone();
		...
		const imageData = matToImageData(copy);
		canvas.height = imageData.height;
		canvas.width = imageData.width;

		await showImageDataInCanvas(matToImageData(copy), canvas.getContext("2d")!);
	}

	return processLandmarks(results);
}



export async function runModelPipelineFromImageData(model: ClassificationModel, inputSize: number, imageData: ImageData) {
	const originalMat = imageDataToMat(imageData);
	...
	// Hacer que el procesamiento de los landmarks se haga en otra función o algo
	// Mirar si se puede generar varios archivos, uno con el procesamiento este
	// otro con el OpenCV
	// y otro con haar
	return model.runModel(tensor); 
}
